    ...
    <header className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-sm">
      <div className="max-w-6xl mx-auto px-6 py-3 flex items-center justify-between">
        <Link to="/" onClick={(e) => handleNavClick(e, '/')} className="flex items-center gap-3 no-underline">
          <img src="/favicon.png" alt="Clean Sport ID Logo" className="h-14 object-contain" />
        </Link>
        <nav className="flex items-center gap-6">
          <Link to="/" onClick={(e) => handleNavClick(e, '/')} className="text-sm font-semibold text-slate-600 hover:text-[#00bcd4] transition">Accueil</Link>
          <Link to="/medications" onClick={(e) => handleNavClick(e, '/medications')} className="text-sm font-semibold text-slate-600 hover:text-[#00bcd4] transition">Médicaments</Link>
          <Link to="/ama" onClick={(e) => handleNavClick(e, '/ama')} className="text-sm font-semibold text-slate-600 hover:text-[#00bcd4] transition">Substances AMA</Link>
        </nav>
      </div>
    </header>
  );
...
// Composant Footer réutilisable en BLANC (Logo agrandi et texte supprimé)
function Footer({ hasAccepted, onTriggerWarning, onResetHome }) {
  ...
  return (
    <footer className="bg-white text-slate-600 pt-16 pb-8 border-t border-slate-200 mt-auto">
      ...
      <div className="max-w-6xl mx-auto px-6 pt-8 border-t border-slate-100 text-center text-xs text-slate-400 flex flex-col sm:flex-row items-center justify-between gap-4">
...
function Home({ showWarning, setShowWarning, accepted, setAccepted, refused, setRefused, pendingRoute, setPendingRoute }) {
  ...
  if (refused) {
    return (
      <div className="flex-grow bg-slate-50 flex flex-col items-center justify-center p-6">
  ...
  if (showWarning && !accepted) {
    return (
      <div className="flex-grow bg-slate-50 flex items-center justify-center p-6 py-12">
        <div className="bg-white p-8 md:p-12 rounded-3xl shadow-xl max-w-2xl text-center border border-slate-100">
          <img src="/favicon.png" alt="Clean Sport ID Logo" className="h-20 mx-auto mb-6 object-contain" />
          ...
          <p className="text-slate-600 mb-8 text-sm md:text-base leading-relaxed text-justify px-2">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <button onClick={() => { setShowWarning(false); setRefused(true); }} className="px-8 py-3.5 border-2 border-slate-200 text-slate-600 font-bold rounded-xl hover:bg-slate-50 hover:border-slate-300 transition w-full sm:w-auto">
              Je refuse
            </button>
            <button 
              onClick={() => { 
              ...
              className="px-8 py-3.5 text-white font-bold rounded-xl hover:opacity-90 transition shadow-md hover:shadow-lg w-full sm:w-auto"
            >
  ...
  if (!accepted) {
    return (
      <div className="flex-grow bg-slate-50 flex items-center justify-between overflow-hidden my-auto">
        <div className="max-w-xl px-8 md:px-16 py-12 z-10">
          ...
          <button 
            ...
            className="inline-flex items-center gap-3 text-white font-bold px-8 py-4 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 text-lg group cursor-pointer"
          >
  ...
  return (
    <div className="flex-grow bg-slate-50 flex flex-col items-center justify-center p-6 py-16">
      <div className="text-center mb-12 max-w-2xl">
        <h1 className="text-3xl font-extrabold text-slate-800 mb-3">Bases de données disponibles</h1>
      ...
      <div className="flex flex-col sm:flex-row gap-6 w-full max-w-3xl">
        <Link 
          to="/medications" 
          className="flex-1 bg-white p-8 rounded-3xl shadow-sm hover:shadow-xl hover:-translate-y-1 border border-slate-200 flex flex-col items-center transition-all duration-300 group no-underline"
        >
          <div 
            className="p-5 rounded-2xl mb-6 transition-colors duration-300"
            style={{ backgroundColor: 'rgba(0, 188, 212, 0.1)' }}
          >
            <ShieldCheck className="w-10 h-10 transition-colors" style={{ color: '#00bcd4' }} />
          </div>
          <h2 className="text-2xl font-bold text-slate-800 mb-2 group-hover:text-[#00bcd4] transition-colors">Médicaments</h2>
        ...
        >
          <div 
            ...
            style={{ backgroundColor: 'rgba(0, 188, 212, 0.1)' }}
          >
            <Info className="w-10 h-10 transition-colors" style={{ color: '#00bcd4' }} />
          </div>
          <h2 className="text-2xl font-bold text-slate-800 mb-2 group-hover:text-[#00bcd4] transition-colors">Substances AMA</h2>
